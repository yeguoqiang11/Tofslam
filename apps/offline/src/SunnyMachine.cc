...
int main(int argc, char** argv) {
    ...
    std::string config_file = "../../config/SunnyMachine1T.json";

    std::shared_ptr<DataIO::DataSetType::SunnyMachine> data_ptr =
        std::make_shared<DataIO::DataSetType::SunnyMachine>(data_folder);

    // slam object
    std::shared_ptr<SlamSystem> slam_ptr = std::make_shared<SlamSystem>(config_file);

    PangoViewer viewer;
    viewer.Start();

    cv::namedWindow("front", cv::WINDOW_NORMAL);
    cv::Mat show(100, 100, CV_8UC1);

    unsigned long count = 0;
    SlamSystem::SensorsData datas;
    double timestamp, timestamp_last = -1;
    while (data_ptr->GetPointCloud(datas.points, timestamp)) {
