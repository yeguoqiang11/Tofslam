...
void TestOptional(std::optional<double> x0) {
    if (x0 == std::nullopt) {
        std::cout << "it is a null optional!!!" << std::endl;
    } else {
        x0.value() = 6;
        std::cout << "optional x: " << x0.value() << std::endl;
    }
}
using namespace slam;
int main(int argc, char** argv) {
    double y = 5;
    std::optional<double> x{7};
    TestOptional(y);
    std::cout << "y: " << y << std::endl;
    std::cout << "transform identity: " << utils::Transform3d::Identity() << std::endl;
    exit(0);
